...
public class ProductService : IProductService
{
    ...
    public async Task<ResponseModel<ProductModel>> CreateAsync(ProductCreateModel productCreateModel)
    {
        ...
        foreach (var CategoryIds in productCreateModel.CategoryIds)
        {
            formData.Add(new StringContent(CategoryIds.ToString()), "CategoryIds");
        }
    ...
    {
        var response = await _httpClientService.GetAsync<ResponseModel<ProductModel>>($"products/get/all{id}");
        return response!;
    ...
    {
        var response = await _httpClientService.DeleteAsync<ResponseModel<NoContent>>($"products/get/hardelete/{id}");
        return response!;
    }

    public async Task<ResponseModel<NoContent>> SoftDeleteAsync(int id)
    {
        var response = await _httpClientService.DeleteAsync<ResponseModel<NoContent>>($"products/get/softdelete/{id}");
        return response!;
