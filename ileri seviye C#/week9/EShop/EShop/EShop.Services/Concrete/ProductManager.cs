...
public class ProductManager : IProductService
{
    ...
    public async Task<ResponseDto<NoContent>> HardDeleteAsync(int id)
    {
        try
        {
            ...
            if (result < 1)
            {
                return ResponseDto<NoContent>.Fail("Ürün silinirken bir hata oluştu.", StatusCodes.Status500InternalServerError);
            }
    ...
    public async Task<ResponseDto<NoContent>> UpdateAsync(ProductUpdateDto productUpdateDto)
    {
        try
        {
            ...
            {
                return ResponseDto<NoContent>.Fail("kategorisi bulunanamadı", StatusCodes.Status400BadRequest);
            }
            //Katerorilerin varlığı kontrol ediliyor.
            ...
            if (productUpdateDto.Image != null)
            {
                ...
                //ürünün eski resmini siliyoruz.
                var deleteResponse = _imageService.DeleteImage(product.ImageUrl!);
                if (!deleteResponse.IsSuccessful)
                {
                    return ResponseDto<NoContent>.Fail(deleteResponse.Error, deleteResponse.StatusCode);
                }
                product.ImageUrl = imageResponse.Data;
            }
